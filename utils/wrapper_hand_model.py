...
class wrapper_hand_model(object):
    def __init__(self, lib_file='./utils/libPythonWrapper.so', model_file='./utils/hand2_l_all_uv.json'):
        ...
        self.func.argtypes = [ctypes.POINTER(ctypes.c_double), ctypes.c_char_p, ctypes.POINTER(ctypes.c_ubyte), ctypes.POINTER(ctypes.c_double), ctypes.POINTER(ctypes.c_double), ctypes.POINTER(ctypes.c_double)]
    ...
    def fit_render(self, joint3d):
        ...
        self.func(self.ctarget_array, self.cmodel_file, self.cret_bytes, self.cpose, self.ccoeff, self.ctrans)
...
if __name__ == '__main__':
    ...
    img, trans, pose, coeff = wrapper.fit_render(joint3d)
    print(trans)
    print(pose)
    print(coeff)
    plt.imshow(img)
