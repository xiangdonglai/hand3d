...
class OpenposeSTBReader(object):
    def __init__(self, mode='evaluation', batch_size=1, shuffle=False, hand_crop=False, use_wrist_coord=False):

        self.batch_size = batch_size
        self.shuffle = shuffle
        assert hand_crop # This data reader is only used to load the Openpose detection result.
        self.hand_crop = hand_crop
        self.use_wrist_coord = use_wrist_coord
        self.sigma = 25.0

        if mode == 'training':
            self.path_to_db = './openpose/openpose_stb_train.pkl'
        else:
            assert mode == 'evaluation'
            self.path_to_db = './openpose/openpose_stb_eval.pkl'
    ...
    def get(self):
        ...
        if not self.use_wrist_coord:
            # see the definition of root in STB dataset! (BinaryDbReaderSTB.py)
            palm_coord_uv = tf.expand_dims(0.5*(keypoint_uv21[0, :] + keypoint_uv21[16, :]), 0)
...
if __name__ == '__main__':
    d = OpenposeSTBReader(mode='evaluation', batch_size=1, shuffle=True, hand_crop=True, use_wrist_coord=False)
    data = d.get()
